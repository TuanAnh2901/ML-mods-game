...
from query_methods import (MethodTarget, atomic_write_text, header_output_path, load_method_entries, load_targets, parse_target, resolve_target)
...
def render_header(rows: Sequence[dict[str, Any]]) -> str:
    lines = ["// Auto-generated by tools/gen_method_fallback.py", "#pragma once", "#include <stdint.h>", "#include <stddef.h>", "", "typedef struct {", "    const char* assembly;", "    const char* ns;", "    const char* klass;", "    const char* method;", "    int argc;", "    uintptr_t rva;", "} MethodFallbackEntry;", "", f"static MethodFallbackEntry g_methodFallbackTable[{len(rows)}] = {{"]
    for row in sorted(rows, key=lambda r: r["rva"]):
        lines.append(f'    {{"{row["assembly"]}", "{row["ns"]}", "{row["cls"]}", "{row["method"]}", {row["argc"]}, 0x{row["rva"]:X}}},')
    lines.extend(["};", "", "static const size_t g_methodFallbackCount = sizeof(g_methodFallbackTable) / sizeof(g_methodFallbackTable[0]);", ""])
    return "\n".join(lines)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    for target in targets:
        ...
        if result.status == "not_found": missing = True; print(f"NOT FOUND: {target.type_name}::{target.method_name}", file=sys.stderr); continue
        if result.status != "resolved": ambiguous = True; print(f"AMBIGUOUS/NON-DECOMPILABLE: {target.type_name}::{target.method_name}", file=sys.stderr); continue
        entry = result.entry
        ns, cls = split_type_name(entry.type_name)
        rows.append({"assembly": target.assembly, "ns": target.namespace_override if target.namespace_override is not None else ns, "cls": cls, "method": target.method_name, "argc": target.argc if target.argc is not None else 0, "rva": entry.rva})
    ...
    if ambiguous: raise SystemExit(5)
