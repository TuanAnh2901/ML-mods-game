...
AGENT = ROOT / "tools" / "bunny_royale_event_frida.js"
DEFAULT_OUT = ROOT / "artifacts" / "bunny_royale_trace" / "frida.jsonl"
...
def write_message(output, message, data):
    ...
    if isinstance(payload, dict):
        ...
        if event in {"candidate_classes", "card_method_catalog", "hook_installed", "card_signal", "trace_ready"}:
            print(json.dumps(payload, ensure_ascii=True), flush=True)


def main() -> int:
    parser = argparse.ArgumentParser(description="Attach a read-only Frida Bunny Royale trace")
    ...
    parser.add_argument("--min-interval-ms", type=int, default=1000, help="Minimum interval per managed callback")
